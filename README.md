# Notice: Work in Progress

# Requirements
ArcGIS Pro v3.3 or greater

# Installation

1. Download repository
	- [Option 1] Using git - Note: this has the benefit of allowing you to pull updates easily any time but is more complicated for non-developers
     	1. Clone repo locally: `git clone https://github.com/bcmertz/SWCD-Tools.git`
	- [Option 2] Download zip - Note: this is the simplest way to get access to these tools
     	1. Download zip file
			- enter this link in your browser to download the zip (https://github.com/bcmertz/SWCD-Tools/archive/refs/heads/main.zip)
			- unzip the folder into a folder where you want it to live
2. in ArcGIS Pro: Catalog -> Toolboxes -> Add Toolbox
	- select `SWCD Tools.pyt` from cloned repository

# Overview of tools (TODO)
## Wetland Tools
### 1. Berm Analysis
![image]("./assets/readme_examples/berm analysis before.png")
![image]("./assets/readme_examples/berm analysis after.png")
### 2. Dam Removal
![image]("./assets/readme_examples/dam removal before.jpg")
![image]("./assets/readme_examples/dam removal after.jpg")
### 3. Model Potential Wetlands
TODO
## Hydrology
### 1. Calculate Streamlines
### 1. EFH-2 Calculation
### 1. Relative Elevation Model (REM)
![image]("./assets/readme_examples/rem after.png")
### 1. Runoff Potential
### 1. Stream Centerline Adjuster
### 1. Stream Elevation Profile
### 1. Sub-Basin Delienation
### 1. Topographic Wetness Index (TWI)
![image]("./assets/readme_examples/twi before.png")
![image]("./assets/readme_examples/twi after.png")
### 1. Watershed Delineation
## Buffer Tools
### 1. Point Plots
### 1. Shrub Clusters
## Automated Agricultural Assessment
### 1. Delineate Parcels
### 2a. Delineate Agland
### 2b. Delineate NonAg
### 2c. Delineate Forest
### 3. Process
### 4. Export Layouts
## Linear Analysis
TODO
## Analyze Area
TODO
## Export Layouts
TODO
### Historical Imagery
TODO
